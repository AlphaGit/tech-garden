...
> So what is [Multi-Query Attention](https://arxiv.org/abs/1911.02150)? Instead of Q, K, and V all being split into separate heads, *only* Q is split. \[...] You might think this would be a serious problem for the model, but it actually has only a small effect on perplexity. (LLMFast)
...
>
> But unlike regular speculative decoding, this is all done not just in a single model, but in a single model *call* using a specially-crafted attention mask. (LLMFast)
...
> Even better, with libraries like [Outlines](https://github.com/outlines-dev/outlines) or [jsonformer](https://github.com/1rgs/jsonformer), you can give the guided generation sampler a schema, and it will sample *within that schema*! (LLMFast)
